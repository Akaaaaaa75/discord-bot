...
client.on('ready', () => {
    const statuses = [
        () => `${client.guilds.cache.size} https://twitch.tv/acty`,
        () => `${client.guilds.cache.reduce((acc, guild) => acc + guild.memberCount, 0)} https://twitch.tv/acty`
    ]
    let i = 0
    setInterval(() => {
        client.user.setActivity(statuses[i](), {type: 'STREAMING',url: 'https://twitch.tv/acty'})
        i = ++i % statuses.length
    },1e4)
})
